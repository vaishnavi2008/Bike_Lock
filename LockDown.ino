...
#define motor1Pin1 27 // PWMA_1
#define motor1Pin2 26 // PWMB_1
#define motor2Pin1 25 // PWMA_2
#define motor2Pin2 33 // PWMB_2
#define motor1enb 32 // EN_1
#define motor2enb 14 // EN_2
#define Button2 16 // if pressed then a match of fingerprint.
#define Button2 16 // if pressed then a match of fingerprint.
#define Button 22 // button is connected with gpio 23
#define wakeUp 4
#define fiveVoltEn 34
...
void setup() {
    ...
    pAdvertising->addServiceUUID(pDeleteSerivce->getUUID());
    ...
    numFingers = finger.numEnrolled();
    Serial.print("Number Enrolled:");
    Serial.println(numFingers);
    pinMode(fiveVoltEn,OUTPUT);
    //digitalWrite(fiveVoltEn,HIGH);
}
...
void loop() {
  ...
  if(enroll == 1){
    Serial.println("I am enrolling");
    int x = finger.enroll(numFingers + 1);
    Serial.println(x);
    if(x == FINGERPRINT_OK){
      numFingers += 1;
    }
  ...
  if(del == 1){
    Serial.println("I am deleting");
    int y = finger.removeall();
    if(y == FINGERPRINT_OK){
      numFingers = 0;
    }
    del = 0;
